...
namespace Interplay.UI
{
    ...
    public static class Tween
    {
        ...
        public static float Evaluate(TweenType type, float startValue, float endValue, float time, float duration, AnimationCurve curve = null)
        {
            switch (type)
            {
				...
				case TweenType.Shake:
					return Shake (startValue, endValue, time, duration);
            }
		...
		/// <summary>
		/// Evaluates a point on a shake tween to return a value.
		/// </summary>
		/// <param name="startValue">The value at the start of the tween.</param>
		/// <param name="endValue">The value at the end of the tween.</param>
		/// <param name="time">The current time of the tween.</param>
		...
		/// The calculated value.
		/// </returns>
		public static float Shake(float startValue, float endValue, float time, float duration)
		{
			float differenceValue = endValue - startValue;
			time = Mathf.Clamp(time, 0f, duration);
			time /= duration;
			...
			return Mathf.PingPong (time*30f, differenceValue) + startValue;
		}
		...
		/// The calculated value.
		/// </returns>
		public static Vector2 Shake(Vector2 startValue, Vector2 endValue, float time, float duration)
		{
			Vector2 tempVector2 = startValue;
			tempVector2.x = Shake(startValue.x, endValue.x, time, duration);
			tempVector2.y = Shake(startValue.y, endValue.y, time, duration);
			return tempVector2;
		}
		...
		/// The calculated value.
		/// </returns>
		public static Vector3 Shake(Vector3 startValue, Vector3 endValue, float time, float duration)
		{
			Vector3 tempVector3 = startValue;
			tempVector3.x = Shake(startValue.x, endValue.x, time, duration);
			tempVector3.y = Shake(startValue.y, endValue.y, time, duration);
			tempVector3.z = Shake(startValue.z, endValue.z, time, duration);
			return tempVector3;
		}
		...
		/// The calculated value.
		/// </returns>
		public static Color Shake(Color startValue, Color endValue, float time, float duration)
		{
			Color tempColor = startValue;
			tempColor.r = Shake(startValue.r, endValue.r, time, duration);
			tempColor.g = Shake(startValue.g, endValue.g, time, duration);
			tempColor.b = Shake(startValue.b, endValue.b, time, duration);
			tempColor.a = Shake(startValue.a, endValue.a, time, duration);
			return tempColor;
		}
    }
}