...
namespace Interplay.UI
{
    ...
    [AddComponentMenu("InterplayUI/Dropdown", 100)]
...
//        }
		...
		private CanvasScaler scaler
        {
            get
            {
                if (m_Scaler == null)
                {
					m_Scaler = transform.root.GetComponent<CanvasScaler> ();
                }
        ...
        protected override void Start()
        {
            ...
            m_DropdownCanvas = m_BaseTransform.GetRootCanvas().Copy(m_DropdownCanvasGameObject);
        }
        ...
        public void Show()
        {
			...
			m_CancelPanel.sizeDelta = transform.root.GetComponent<Canvas> ().pixelRect.size * 2;// CHANGED FROM: scaler.targetCanvas.pixelRect.size * 2;
            ...
            {
				m_TempMaxHeight = transform.root.GetComponent<RectTransform>().rect.height - 32; // CHANGED FROM: MaterialUIScaler.GetParentScaler(m_DropdownPanel).targetCanvas.GetComponent<RectTransform>().rect.height - 32;
            }
        ...
        private Vector3 CalculatedPosition()
        {
			...
			RectTransform rootCanvasRectTransform = transform.root.GetComponent<RectTransform>();
        ...
        private float CalculateMaxItemWidth()
        {
            ...
            for (int i = 0; i < m_OptionDataList.options.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(m_OptionDataList.options[i].text))
                {
                    ...
                    //if (m_DropdownCanvas.renderMode != RenderMode.WorldSpace)
                    //{
                    //    currentWidth /= scaler.targetCanvas.scaleFactor;
                    //}
                    currentWidth += 16;
                }
