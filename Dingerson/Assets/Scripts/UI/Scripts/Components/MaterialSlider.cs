...
//    [ExecuteInEditMode]
    [AddComponentMenu("InterplayUI/Material Slider", 100)]
...
//                    }
					m_RootCanvas = transform.root.GetComponent<Canvas> ();
                }
                return m_RootCanvas;
            }
        }
            ...
            if (m_HasPopup)
            {
                m_HandlePositionYTweener =
                    TweenManager.TweenFloat(
						...
						transform.root.GetComponent<Canvas>().pixelPerfect ? 1f : 0f, m_AnimationDuration, 0, null, false, Tween.TweenType.EaseOutCubed);
